...
class Helpers
{
    ...
    public static function create_wizgo_customer($user)
    {
        $baseUrl = 'https://app.wigzo.com/rest/v1/learn/identify';
        $queryParams = [
        ...
        $payload = [
            'userId' => $user->uuid,
            'email' => $user->email ?? 'fastemiecommerce@gmail.com',
            'phone' => $user->phone,
            'is_active' => true,
            'source' => 'web',
        ...
        Log::info('create_wigzo_customer event - event url --'.$fullUrl);
        Log::info('create_wigzo_customer event - payload  --'.json_encode($payload));
    ...
    public static function wizgo_event_send($user_id, $event_name, $eventVal)
    {
        $queryParams = [
            'token' => env('WIGZO_TOKEN'),
            'org_token' => env('WIGZO_ORG_TOKEN')
        ];

        $fullUrl = 'https://app.wigzo.com/rest/v1/learn/event' . '?' . http_build_query($queryParams);
        ...
        Log::info($event_name.' - event url --'.$fullUrl);
        Log::info($event_name.' - payload  --'.json_encode($payload));
        $response = Http::withHeaders([
            'Content-Type' => 'application/json',
        ])->post($fullUrl,$payload);
        ...
        if ($response->successful()) {
            // Handle successful response
            Log::info('Event Response: '.$event_name.'---'. json_encode($response->json()));
        } else {
            // Handle error response
    ...
    public static function sms_event_priority($event, $id, $payload=Null): string
    {
        Log::info('sms_event_priority function called -');

        $config = self::get_config_settings('priority_sms');
        $response = 'error';
        ...
        if (isset($config) && $config['status'] == 1) {
            try {
                ...
                if(isset($user->f_name)){
                    $username = ucfirst($user->f_name);
                }

                if($event == 'confirmed'){
                    $otpTemplate = 'Hello $username, your order number $order_no of $product_detail has been successfully placed with FASTEMI by ZEBRS INDIA OPC';
                    $product_detail = 'BUYWITHEMI Product';
                    $order_no = isset($payload['order_no']) ? $payload['order_no'] : '00001';

                    // Replace the placeholder in the template with the actual OTP
                    $message = str_replace(
                        ['$username', '$order_no','$product_detail'],
                        [$username, $order_no, $product_detail],
                        $otpTemplate
                    );
                } else if($event == 'checkout'){
                    $otpTemplate = 'Hello $username, the product you selected is still in your cart. Please complete the order with our express checkout FASTEMI by ZEBRS INDIA OPC';
                    ...
                    $otpTemplate = 'Hello $username, your order number $order_no of $product_detail has been cancelled. You can place another order at FASTEMI by ZEBRS INDIA OPC';
                    $product_detail = 'BUYWITHEMI Product';
                    $order_no = isset($payload['order_no']) ? $payload['order_no'] : '00001';
                    ...
                    $otpTemplate = 'Hello $username, your order number $order_no of $product_detail has been successfully processed with FASTEMI by ZEBRS INDIA OPC';
                    $product_detail = 'BUYWITHEMI Product';
                    $order_no = isset($payload['order_no']) ? $payload['order_no'] : '00001';
                    ...
                    $otpTemplate = 'Hello $username, your order number $order_no of $product_detail has been delivered. Thank you for shopping with FASTEMI by ZEBRS INDIA OPC';
                    $product_detail = 'BUYWITHEMI Product';
                    $order_no = isset($payload['order_no']) ? $payload['order_no'] : '00001';
                ...
                // Sending OTP via Priority SMS Gateway
                $prioritySmsUrl = 'https://alerts.prioritysms.com/api/web2sms.php';
                $smsParams = [
                    'workingkey' => $config['api_key'],
                    'sender'     => $config['sender'],
                    'to'         => $receiver,
                    'message'    => $message
                ];

                Log::info('Priority SMS URL: ' . $prioritySmsUrl . ' with params ' . json_encode($smsParams));

                $smsResponse = Http::get($prioritySmsUrl, $smsParams);

                if ($smsResponse->successful()) {
                    Log::info('Priority SMS sent successfully.');
                    $response = 'success';
                } else {
                    Log::error('Priority SMS failed: ' . $smsResponse->body());
                    $response = 'error';
                }

            } catch (\Exception $exception) {
                Log::error('Exception in priority_sms: ' . $exception->getMessage());
                $response = 'error';
            }
        }
    ...
    public static function get_config_settings($name)
    {
...
//        return (($price-$discount) / 100) * $tax; //after discount decrease
    ...
    */
    ...
    public static function get_customer_via_id($id)
    {
        return User::find($id);
    }
