...
class CartController extends BaseController
{
    ...
    {
        $html = '';
        $carts = ModelsCart::where('customer_id', $request->customer_id)->get();
        ...
        // Loop through the products and add table rows
        if ($carts->isEmpty()) {
            $html .= '<tr><td colspan="4">No products found for this customer.</td></tr>';
        } else {
            foreach ($carts as $cart) {
                $html .= '<tr>';
                $html .= '<td><img src="' . $cart->allProducts->thumbnail_full_url['path'] . '" alt="Product Image" style="width: 75px; height: auto;"></td>';
                $html .= '<td>' . $cart->name . '</td>'; // Assuming you have a 'name' field
                $html .= '<td>' . $cart->quantity . '</td>'; // Assuming you have a 'quantity' field
                $html .= '<td> ₹' . $cart->price . '</td>'; // Assuming you have a 'price' field
                $html .= '</tr>';
            }
        }
