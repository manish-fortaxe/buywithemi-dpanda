...
class HomeController extends Controller
{
    ...
    public function default_theme(): View
    {
        ...
        $brand_setting = Cache::remember('brand_setting', 60, function() {
        ...
        $brands = Cache::remember('brands', 60, function() {
            return Brand::active()->take(15)->orderBy('sequence', 'desc')->get();
        });

        $main_banner = Cache::remember('main_banner', 60, function() use ($theme_name) {
            return $this->banner->where(['banner_type' => 'Main Banner', 'theme' => $theme_name, 'published' => 1])->latest()->get();
        });

        $main_banner_right = Cache::remember('main_banner_right', 60, function() use ($theme_name) {
            return $this->banner->where(['banner_type' => 'Main Banner Right', 'theme' => $theme_name, 'published' => 1])->latest()->take(2)->get();
        });

        // $home_category_banner = Cache::remember('home_category_banner', 60, function() use ($theme_name) {
        //     return $this->banner->where(['banner_type' => 'Home Category Banner', 'theme' => $theme_name, 'published' => 1])->orderBy('sequence')->get();
        // });

        $main_section_banner = Cache::remember('main_section_banner', 60, function() use ($theme_name) {
            return $this->banner->where(['banner_type' => 'Main Section Banner', 'theme' => $theme_name, 'published' => 1])->orderBy('id', 'desc')->latest()->first();
        });

        $footer_banner = Cache::remember('footer_banner', 60, function() use ($theme_name) {
            return $this->banner->where('banner_type', 'Footer Banner')->where('theme', $theme_name)->where('published', 1)->orderBy('id', 'desc')->get();
        });
        ...
        compact(  'categories', 'brands',
            'homeCategories', 'brand_setting', 'main_banner', 'main_section_banner',
            'current_date',  'footer_banner', 'main_banner_right','blogs'
        )
    );
    }
    ...
    public function default_theme(): View
    {
        ...
        $featuredProductsList = ProductManager::getPriorityWiseFeaturedProductsQuery(query: $this->product->active(), dataLimit: 10);

        $latest_products = $this->product->with(['reviews'])->active()->orderBy('id', 'desc')->take(10)->get();
        ...
        $main_banner_right = $this->banner->where(['banner_type' => 'Main Banner Right', 'theme' => $theme_name, 'published' => 1])->latest()->take(2)->get();
        $home_category_banner = $this->banner->where(['banner_type' => 'Home Category Banner', 'theme' => $theme_name, 'published' => 1])->orderBy('sequence')->get();
