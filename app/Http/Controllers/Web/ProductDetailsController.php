...
class ProductDetailsController extends Controller
{
    ...
    {
        // Cache the product data with relations for 60 minutes
        $product = Cache::remember("product_with_slug_{$slug}", 60, function () use ($slug) {
            return $this->productRepo->getFirstWhereActive(
                params: ['slug' => $slug],
                relations: ['seoInfo', 'digitalVariation', 'reviews', 'seller.shop']
            );
        });

        if ($product) {
            // Cache decimal point settings for 60 minutes
            $decimalPointSettings = Cache::remember('decimal_point_settings', 60, function () {
                return getWebConfig('decimal_point_settings');
            });

            // Cache related products based on category, subcategory, and sub-subcategory for 60 minutes
            $relatedProducts = Cache::remember("related_products_for_product_{$product['id']}", 60, function () use ($product) {
                $relatedProductsQuery = Product::query()->active();

                // Apply filters based on product's category, subcategory, and sub-subcategory
                if (!empty($product['category_id'])) {
                    $relatedProductsQuery->where('category_id', $product['category_id']);
                }
                if (!empty($product['sub_category_id'])) {
                    $relatedProductsQuery->where('sub_category_id', $product['sub_category_id']);
                }
                if (!empty($product['sub_sub_category_id'])) {
                    $relatedProductsQuery->where('sub_sub_category_id', $product['sub_sub_category_id']);
                }

                // Exclude the current product by ID and limit results
                return $relatedProductsQuery->whereNotIn('id', [$product['id']])
                                            ->limit(10)
                                            ->get();
            });

            // Cache vacation settings for 60 minutes
            $temporaryClose = Cache::remember('temporary_close', 60, function () {
                return getWebConfig('temporary_close');
            });
            $inHouseVacation = Cache::remember('vacation_add', 60, function () {
                return getWebConfig('vacation_add');
            });
