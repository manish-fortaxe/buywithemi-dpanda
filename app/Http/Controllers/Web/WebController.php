...
use App\Enums\ViewPaths\Web\Product as WebProduct;
...
class WebController extends Controller
{
    ...
    public function getAllCategoriesView(): View
    {
        ...
        if(auth('customer')->user()){
            ...
            // Helpers::wizgo_event_send(auth('customer')->user()->uuid,'‘categoryview', $data);
        }
    ...
    public function checkout_details(Request $request)
    {
        if (
            ...
            return redirect()->route('customer.auth.login',['intend'=>'checkout-details']);
        }
        ...
        if(auth('customer')->user()){
            // Helpers::wizgo_event_send(auth('customer')->user()->uuid,'checkoutstarted', $data);
            // Helpers::sms_event_priority('checkout', auth('customer')->user()->id);
        }
    ...
    public function order_placed(): View
    {
        ...
        //         'product2'
        //     ];
        //     Helpers::wizgo_event_send(auth('customer')->user()->uuid,'buy', $data);
        // }
    ...
    public function shop_cart(Request $request): View|RedirectResponse
    {
        if (
            ...
            {
                $carts= Cart::where(['customer_id' => auth('customer')->id()])->get()->groupBy('cart_group_id');
                foreach($carts as $key=>$value)
                {
                    $shipping = CartShipping::where(['cart_group_id' => $key])->first();
                    if (!isset($shipping)) {
                        $shipping = new CartShipping();
                    }
                    $shippingMethod = ShippingMethod::where('status', 1)->first();
                    $shipping['cart_group_id'] = $key;
                    $shipping['shipping_method_id'] = $shippingMethod->id;
                    $shipping['shipping_cost'] = $shippingMethod->cost ?? 0;
                    $shipping->save();
                }
            }
    ...
    public function google_merchant_upload(){
        $filePath = 'feeds/google_merchant_feed.xml';
        ...
        if (Storage::exists($filePath)) {
            ...
            return Response::make('XML file not found.', 404);
        }
    }
}
