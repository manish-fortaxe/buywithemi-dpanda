...
trait  SmsGateway
{
    public static function send($receiver, $otp, $id = Null): string
    {
        $config = self::get_settings('priority_sms');
        Log::info('sms gateway priority_sms status -'.json_encode($config));
        if (isset($config) && $config['status'] == 1) {
            return self::priority_sms($receiver, $otp, $id);
        }

        $config = self::get_settings('wizgo');
        Log::info('sms gateway config status -'.json_encode($config));
        if (isset($config) && $config['status'] == 1) {
            return self::wizgo($receiver, $otp, $id);
        }
    ...
    public static function wizgo($receiver, $otp, $id): string
    {
        Log::info('sms gateway wizgo function called -');
        ...
        if (isset($config) && $config['status'] == 1) {
            ...
            try {
                ...
                Log::info('send_otp event - event url --'.$fullUrl);
                Log::info('send_otp event - payload  --'.json_encode($payload));
                ...
                if ($response->successful()) {
                    Log::info('Event Response: send_otp ---'. json_encode($response->json()));
    ...
    public static function priority_sms($receiver, $otp, $id): string
    {
        Log::info('sms gateway priority_sms function called -');
        ...
        if (isset($config) && $config['status'] == 1) {
            try {
                $user = User::find($id);
                $username = $user->phone;
                // Retrieve OTP template from the backend
