    ...
    protected function sendOrderNotification(string $key, string $type, object|array $order): void
    {
        try {
            ...
            if ($type == 'customer') {
                ...
                //     // $this->sendPushNotificationToDevice($fcmToken, $data);
                //     // Helpers::wizgo_event_send($order->customer->uuid,'order', $data);
                // }
                Log::info('push notification trait event key -'.$key);
                //Priority SMS Alerts
                $smspayload = [
                    'order_no' => $order['id']
                ];
                if(in_array($key, ['confirmed', 'processing', 'canceled' ,'delivered'])){
                    // Helpers::sms_event_priority($key, $order->customer->id, $smspayload);
                }
            }
            /** end for customer  */
            /**for seller */
