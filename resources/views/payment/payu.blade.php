...
@section('content')
    ...
    <form action="{{ $url }}" id="payment_form_submit" method="post">
        ...
        @isset($enforce_paymethod)
        <input type="hidden" name="enforce_paymethod" value="{{ $enforce_paymethod }}" />
        @endisset
        @if(isset($pg) && isset($bank_codes))
        <input type="hidden" name="pg" value="{{ $pg }}" />
        <input type="hidden" name="bankcode" value="{{ $bank_codes }}" />
        @endif
    </form>
